...
// document attributes
:dev-center: https://app.corva.ai/dev-center/docs[Corva DevCenter]
:corva-platform-api: https://api.corva.ai/documentation/index.html[Corva Platform API]
:corva-data-api: https://data.corva.ai/docs#/[Corva Data API]
:api-v1-data-provider-dataset: https://data.corva.ai/docs#/default/index_api_v1_data__provider___dataset___get[/api/v1/data/provider/dataset/]
:log-levels: https://docs.python.org/3/howto/logging.html[Python log levels]
...
Documentation for version: *v0.0.18*.

{corva-sdk} is a framework for building {dev-center} Python apps.
...
TIP: Use type hints like those in the examples below
...
----
<.> Use {corva-platform-api} url suffix
to make the Platform API call.
<.> Use {corva-data-api} url suffix
to make the Data API call.
...
A typical example of `Cache` usage could be:
...
* Logging level can be set using `LOG_LEVEL` env variable.
  ...
  see {log-levels} for other available options.
